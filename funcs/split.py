...
def split(DIM, OFF, BAR):
    ...
    mouse = list(map(lambda x: int(x[2:]), call("xdotool getmouselocation").split()[:2]))
    ...
    if DIM == "vertical":
        for window in window_list:
            ...
            if left(window, display, offset):
                print("%d is left-aligned, adjusting" % window[0])
            ...
            if right(window, display, offset):
                print("%d is right-aligned, adjusting" % window[0])
        ...
        for window in window_list:
            ...
            if top(window, display, offset):
                print("%d is top-aligned, adjusting" % window[0])
                call("xdotool windowsize %d %d %d --sync" % (window[0], display[0], mouse[1] - offset[1]))
                call("xdotool windowmove %d %d %d --sync" % (window[0], offset[0], offset[1]))
                continue 
            # Check if window is aligned to the right:
            if bottom(window, display, offset):
                print("%d is bottom-aligned, adjusting" % window[0])
                call("xdotool windowsize %d %d %d --sync" % (window[0], display[0], display[1] - mouse[1] + offset[1]))
                call("xdotool windowmove %d %d %d --sync" % (window[0], offset[0], mouse[1]))
                continue
